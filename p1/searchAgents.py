...
class CornersProblem(SearchProblem):
    ...
    def __init__(self, startingGameState):
        ...
        self.costFn = distance.manhattan
    ...
    def successorStates(self, state):
        ...
        for action in Directions.CARDINAL:
            ...
            if (not hitsWall):
                remainingCorners = tuple([x for x in state[2] if x!=(state[0], state[1])])
                nextState = (nextx, nexty, remainingCorners)
                
                # mincost = 0
                # for corner in self.cornersRemain:
                #     mincost += self.costFn(state, corner)
                # successors.append((nextState, action, cornersHeuristic(state, self)))
...
def cornersHeuristic(state, problem):
    ...
    # find distnace to closest corner
    minCorner = []
    for corner in reminaingCorners:
        cornerDistance = distance.maze((state[0], state[1]), corner, problem.gameState)
        if len(minCorner) == 0:
            minCorner = [corner, cornerDistance]
        elif cornerDistance < minCorner[1]:
            minCorner = [corner, cornerDistance]

    # find distance from corner to other corners
    for corner in reminaingCorners:
        minCorner[1] += distance.maze(minCorner[0], corner, problem.gameState)

        # print(str((state[0], state[1])) + str(corner) + str(cost) + str(distance.maze((state[0], state[1]), corner, problem.gameState)))
...
def foodHeuristic(state, problem):
    ...
    for food in foodGrid.asList():
        foodDist = distance.maze((position[0], position[1]), food, problem.startingGameState)
        if len(minFood) == 0:
            minFood = [food, foodDist]
        elif foodDist < minFood[1]:
            minFood = [food, foodDist]

    
    for food in foodGrid.asList():
        minFood[1] += distance.maze(minFood[0], food, problem.startingGameState)
